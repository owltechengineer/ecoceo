...
// Funzioni per statistiche globali dei progetti
export const getGlobalProjectStats = async (): Promise<{
  ...
  try {
    ...
    if (projectsError) {
      throw new Error(`Errore caricamento progetti: ${projectsError.message}`);
    }
    ...
    // Carica statistiche aggregate per rischi e milestone
    let criticalRisks = 0;
    let overdueMilestones = 0;
    let totalTeamAllocation = 0;

    if (projects && projects.length > 0) {
      const projectIds = projects.map(p => p.id);
      
      // Carica tutti i rischi critici
      ...
      const { data: milestones } = await supabase
        ...
        .in('project_id', projectIds)
        .eq('status', 'pending')
        .lt('planned_date', now.toISOString().split('T')[0]);

      overdueMilestones = milestones?.length || 0;

      // Calcola utilizzo team
      ...
      totalTeamAllocation = teamMembers?.reduce((sum, member) => sum + member.allocation_percentage, 0) || 0;
    }

    const teamUtilization = totalTeamAllocation;
    ...
    console.error('Errore nel calcolo statistiche globali progetti:', error);
    throw error;
  }
};

// Funzione per ottenere progetti attivi con dettagli
export const getActiveProjectsWithDetails = async (): Promise<Array<{
  project: any;
  stats: any;
  nextMilestone?: any;
  criticalRisks: number;
  ...
  try {
    ...
    if (projectsError) {
      throw new Error(`Errore caricamento progetti attivi: ${projectsError.message}`);
    }
    ...
    // Per ogni progetto, carica statistiche e dettagli
    const projectsWithDetails = await Promise.all(
      projects.map(async (project) => {
        try {
          const [stats, milestones, risks] = await Promise.all([
            getProjectStats(project.id),
            loadProjectMilestones(project.id),
            loadProjectRisks(project.id)
          ]);

          // Trova il prossimo milestone
          const now = new Date();
          const upcomingMilestones = milestones
            .filter(m => m.status === 'pending' && new Date(m.planned_date) > now)
          ...
          console.error(`Errore caricamento dettagli progetto ${project.id}:`, error);
          return {
            project,
            stats: null,
            nextMilestone: null,
    ...
    console.error('Errore nel caricamento progetti attivi con dettagli:', error);
    throw error;
  }
};
...
// Interfacce allineate con docs/database/COMPLETE_DATABASE_ALL_TABLES.sql
...
// ===== WAREHOUSE FUNCTIONS =====
// Funzioni per la gestione del magazzino

export const loadWarehouseItems = async (userId: string = 'default-user'): Promise<WarehouseItem[]> => {
  try {
    const { data, error } = await supabase
    ...
    if (error) {
      console.error('Errore caricamento warehouse items:', error);
      throw new Error(`Errore caricamento warehouse items: ${error.message}`);
    }
...
export const saveWarehouseItem = async (item: Omit<WarehouseItem, 'id' | 'created_at' | 'updated_at'>): Promise<WarehouseItem> => {
  try {
    console.log('🔄 Tentativo salvataggio warehouse item:', item);
    ...
    if (error) {
      console.error('❌ Errore salvataggio warehouse item:', {
        message: error.message,
        details: error.details,
        hint: error.hint,
        code: error.code,
      ...
      throw new Error(`Errore salvataggio warehouse item: ${error.message}`);
    }

    console.log('✅ Warehouse item salvato con successo:', data);
    return data;
  } catch (error: any) {
    console.error('❌ Errore generale salvataggio warehouse item:', error);
    throw new Error(`Errore salvataggio: ${error.message || 'Errore sconosciuto'}`);
  }
};

export const updateWarehouseItem = async (id: string, updates: Partial<WarehouseItem>): Promise<WarehouseItem> => {
  try {
    console.log('🔄 Tentativo aggiornamento warehouse item:', { id, updates });
    ...
    if (error) {
      console.error('❌ Errore aggiornamento warehouse item:', {
        message: error.message,
        details: error.details,
        hint: error.hint,
        code: error.code,
      ...
      throw new Error(`Errore aggiornamento warehouse item: ${error.message}`);
    }

    console.log('✅ Warehouse item aggiornato con successo:', data);
    return data;
  } catch (error: any) {
    console.error('❌ Errore generale aggiornamento warehouse item:', error);
    throw new Error(`Errore aggiornamento: ${error.message || 'Errore sconosciuto'}`);
  }
};

// Funzione alternativa per disabilitare un articolo invece di eliminarlo
export const disableWarehouseItem = async (id: string): Promise<void> => {
  try {
    console.log('🔄 Disabilitazione warehouse item:', id);
    
    const { error } = await supabase
      .from('warehouse_items')
      .update({ 
        quantity: 0,
        description: (await supabase.from('warehouse_items').select('description').eq('id', id).single()).data?.description + ' [DISABILITATO]' || '[DISABILITATO]'
      })
      .eq('id', id);

    if (error) {
      console.error('❌ Errore disabilitazione warehouse item:', error);
      throw new Error(`Errore disabilitazione warehouse item: ${error.message}`);
    }

    console.log('✅ Warehouse item disabilitato con successo');
  } catch (error: any) {
    console.error('❌ Errore generale disabilitazione warehouse item:', error);
    throw new Error(`Errore disabilitazione: ${error.message || 'Errore sconosciuto'}`);
  }
};

export const deleteWarehouseItem = async (id: string): Promise<void> => {
  try {
    console.log('🔄 Tentativo eliminazione warehouse item:', id);
    ...
    if (quoteItemsError) {
      console.error('❌ Errore verifica quote_items:', quoteItemsError);
      throw new Error(`Errore verifica quote_items: ${quoteItemsError.message}`);
    }
    ...
    if (transactionsError) {
      console.error('❌ Errore verifica warehouse_transactions:', transactionsError);
      throw new Error(`Errore verifica warehouse_transactions: ${transactionsError.message}`);
    }

    // Log delle dipendenze trovate
    if (quoteItems && quoteItems.length > 0) {
      console.log(`⚠️ Articolo referenziato in ${quoteItems.length} preventivi`);
    }
    
    if (transactions && transactions.length > 0) {
      console.log(`⚠️ Articolo ha ${transactions.length} transazioni di magazzino`);
    }

    // Elimina le dipendenze in ordine
    if (quoteItems && quoteItems.length > 0) {
      console.log('🔄 Eliminazione quote_items associati...');
      ...
      if (deleteQuoteItemsError) {
        console.error('❌ Errore eliminazione quote_items:', deleteQuoteItemsError);
        throw new Error(`Errore eliminazione quote_items: ${deleteQuoteItemsError.message}`);
      }
      console.log('✅ Quote_items eliminati con successo');
    }

    if (transactions && transactions.length > 0) {
      console.log('🔄 Eliminazione warehouse_transactions associate...');
      ...
      if (deleteTransactionsError) {
        console.error('❌ Errore eliminazione warehouse_transactions:', deleteTransactionsError);
        throw new Error(`Errore eliminazione warehouse_transactions: ${deleteTransactionsError.message}`);
      }
      console.log('✅ Warehouse_transactions eliminate con successo');
    }

    // Ora elimina l'articolo del magazzino
    console.log('🔄 Eliminazione warehouse_item...');
    const { error } = await supabase
      .from('warehouse_items')
      .delete()
      .eq('id', id);
    ...
    if (error) {
      console.error('❌ Errore eliminazione warehouse item:', {
        message: error.message,
        details: error.details,
        hint: error.hint,
        code: error.code,
      ...
      throw new Error(`Errore eliminazione warehouse item: ${error.message}`);
    }

    console.log('✅ Warehouse item eliminato con successo');
  } catch (error: any) {
    console.error('❌ Errore generale eliminazione warehouse item:', error);
    throw new Error(`Errore eliminazione: ${error.message || 'Errore sconosciuto'}`);
  }
};

export const loadWarehouseCategories = async (): Promise<WarehouseCategory[]> => {
  try {
    const { data, error } = await supabase
    ...
    if (error) {
      console.error('Errore caricamento warehouse categories:', error);
      throw new Error(`Errore caricamento warehouse categories: ${error.message}`);
    }
...
export const loadWarehouseLocations = async (): Promise<WarehouseLocation[]> => {
  try {
    const { data, error } = await supabase
    ...
    if (error) {
      console.error('Errore caricamento warehouse locations:', error);
      throw new Error(`Errore caricamento warehouse locations: ${error.message}`);
    }
...
export const loadQuotes = async (userId: string = 'default-user'): Promise<Quote[]> => {
  try {
    const { data, error } = await supabase
      .from('quotes')
      .select('*')
    ...
    if (error) {
      ...
      throw new Error(`Errore caricamento quotes: ${error.message}`);
    }
...
export const saveQuote = async (quote: Omit<Quote, 'id' | 'created_at' | 'updated_at'>): Promise<Quote> => {
  try {
    const { data, error } = await supabase
      .from('quotes')
      .insert([quote])
    ...
    if (error) {
      console.error('Errore salvataggio quote:', error);
      throw new Error(`Errore salvataggio quote: ${error.message}`);
    }
...
export const updateQuote = async (id: string, updates: Partial<Quote>): Promise<Quote> => {
  try {
    ...
    if (error) {
      console.error('Errore aggiornamento quote:', error);
      throw new Error(`Errore aggiornamento quote: ${error.message}`);
    }
...
export const deleteQuote = async (id: string): Promise<void> => {
  try {
    ...
    if (error) {
      console.error('Errore eliminazione quote:', error);
      throw new Error(`Errore eliminazione quote: ${error.message}`);
    }
  } catch (error) {
    console.error('Errore eliminazione quote:', error);
    throw error;
...
export const loadQuoteItems = async (quoteId: string): Promise<QuoteItem[]> => {
  try {
    const { data, error } = await supabase
      .from('quote_items')
      .select('*')
    ...
    if (error) {
      console.error('Errore caricamento quote items:', error);
      throw new Error(`Errore caricamento quote items: ${error.message}`);
    }
...
export const saveQuoteItem = async (item: Omit<QuoteItem, 'id' | 'created_at' | 'updated_at'>): Promise<QuoteItem> => {
  try {
    const { data, error } = await supabase
      .from('quote_items')
      .insert([item])
    ...
    if (error) {
      console.error('Errore salvataggio quote item:', error);
      throw new Error(`Errore salvataggio quote item: ${error.message}`);
    }
...
export const updateQuoteItem = async (id: string, updates: Partial<QuoteItem>): Promise<QuoteItem> => {
  try {
    const { data, error } = await supabase
      .from('quote_items')
      .update(updates)
    ...
    if (error) {
      console.error('Errore aggiornamento quote item:', error);
      throw new Error(`Errore aggiornamento quote item: ${error.message}`);
    }
...
export const deleteQuoteItem = async (id: string): Promise<void> => {
  try {
    const { error } = await supabase
      .from('quote_items')
      .delete()
    ...
    if (error) {
      console.error('Errore eliminazione quote item:', error);
      throw new Error(`Errore eliminazione quote item: ${error.message}`);
    }
  } catch (error) {
    console.error('Errore eliminazione quote item:', error);
    throw error;
...
export const loadWarehouseTransactions = async (itemId?: string, userId: string = 'default-user'): Promise<WarehouseTransaction[]> => {
  try {
    let query = supabase
      .from('warehouse_transactions')
      .select('*')
    ...
    if (error) {
      console.error('Errore caricamento warehouse transactions:', error);
      throw new Error(`Errore caricamento warehouse transactions: ${error.message}`);
    }
...
export const saveWarehouseTransaction = async (transaction: Omit<WarehouseTransaction, 'id' | 'created_at'>): Promise<WarehouseTransaction> => {
  try {
    const { data, error } = await supabase
      .from('warehouse_transactions')
      .insert([transaction])
    ...
    if (error) {
      console.error('Errore salvataggio warehouse transaction:', error);
      throw new Error(`Errore salvataggio warehouse transaction: ${error.message}`);
    }
